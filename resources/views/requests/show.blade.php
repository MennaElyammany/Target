        ...
        <div class="col-md-8">
            <div class="card-body">
                ...
                <form class="form-inline" method="POST" action="/requests/{{$request->id}}">
                    ...
                    <a class="btn btn-outline-danger my-3 " href="/requests/decline/{{$request->id}}"
                        role="button">Decline</a>
                    @elseif($request->status =='accepted')
                    <label class="text-danger">Notify Client When You Are Completed </label>
                    <a class="btn btn-outline-success my-3 " href="/requests/completed/{{$request->id}}"
                    ...
                    <input id="input-1-ltr-star-xs" name="input-1-ltr-star-xs"
                        class="kv-ltr-theme-svg-star rating-loading" value="1" dir="ltr" data-size="xs">

                    @else

                    <a class="btn btn-outline-primary my-3  " href="/requests/accept/{{$request->id}}"
...
<div class="card mb-3 " style="max-width: 600px; position:absolute;left:25%">
    <div class="row no-gutters">
        ...
        <div class="col-md-9">
            <div class="card-body">
                <h5 class="card-title font-weight-bold text-primary">Request From {{$request->company_name}} To
                    {{findUserName($request->influencer_id)}}</h5>
                <p class="font-weight-bold ">Description:
                    <span class="float-right"><a href="{{$request->website_url}}">Visit Website</a>
                    </span></p>
                <div class="border">
